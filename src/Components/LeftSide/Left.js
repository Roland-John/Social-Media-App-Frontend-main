          ...
          <div className="user-name-userid">
            <img src={profileImg ? (profileImg) : Profile} alt="" />
              <div className='L-user'>
                <h1>{modelDetails ? (modelDetails.ModelName) : "User"}</h1>
                <span>{modelDetails ? (modelDetails.ModelUserName) : "@User123"}</span>
            </div>
          </div>
        </Link>
        <MoreHorizIcon onClick={()=>setLogOutExit(!logOutExit)} className='vert'/>
          ...
          {logOutExit && (
            <div className="logOutExitContainer">
              ...
              <Link to="/" style={{width:"100%"}}><button>Log out @User</button></Link>
            </div>
          )}
      </div>
